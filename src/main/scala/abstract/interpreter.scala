...
import scala.collection.mutable.{ HashMap, HashSet, PriorityQueue, Map ⇒ MMap, Set ⇒ MSet }
import java.io._
...
object StatsDeux {
  ...
  val dumbErrors = MSet[Int]()


  def dumbError(e:Exp, ρ:Env, σ:Store, ß:Scratchpad ) {
    if (notJS.Mutable.dumb && notJS.Mutable.inPostFixpoint) {
      e match {
        case Unop(IsPrim, ein) ⇒ {
          if (Eval.eval(e, ρ, σ, ß).b == Bool.⊤) {
            val bv = Eval.eval(ein, ρ, σ, ß)
            if ((bv.nil == Null.⊤) || (bv.undef == Undef.⊤)){
              dumbErrors += e.id
            }
          }
        }
  ...
  def report {
    ...
    // count typeErrors only
    val typeerrs = exceptions.foldLeft( 0 )( 
      (acc, idvs) ⇒ {
        assert( idvs._2.size <= 2 )
        if (idvs._2.size == 2) acc + 1
        else idvs._2.head match {
          case `typeError` ⇒ acc + 1
          case `rangeError` ⇒ acc
          case _ ⇒ sys.error("unexpected exception value")
      }})

    val dumb = if (notJS.Mutable.dumb) dumbErrors.size else 0

    println("NUMBER OF TYPE ERRORS: " + (typeerrs + dumb))    
  }
...
object notJS {
  ...
  object Mutable {
    ...
    var dangle = false      // !! TODO: ???

    // debugging command-line flags
    var testing = false     // !!TODO: ???
    ...
    // begin old flags (don't use)

    // track various sorts of stats (no/yes)
    var stats = false
    var dumb = false
    var typeSize = false

    // do branch refinement in general (no/yes)
    var branchRefine = false

    // do boolean refinement
    var refBool = false

    // do isPrim refinement
    var refIsPrim = false

    // do ToBool refinement
    var refToBool = false
    // do typeof refinement
    var refTypeof = false

    // do exception refinement (no/yes)
    var excRefine = false

    // end old flags

    // in general, do refinement
    ...
    def clear {
      ...
      Mutable.stats = false
      Mutable.dumb = false
      Mutable.typeSize = false
      Mutable.branchRefine = false
      Mutable.refBool = false
      Mutable.refIsPrim = false
      Mutable.refTypeof = false
      Mutable.excRefine = false
      Mutable.inPostFixpoint = false
      Mutable.refToBool = false
      Mutable.refinement = false
  ...
  def runner( args:Array[String] ): MMap[Int, Set[BValue]] = {
    ...
    Mutable.stats      = opts("--stats=branch")
    Mutable.dumb       = opts("--stats=dumb")
    Mutable.typeSize   = opts("--stats=size")
    StatsDeux.trackStats = opts("--estats")
    Mutable.branchRefine = opts("--refbranch")
    Mutable.refBool    = opts("--refbool")
    Mutable.refToBool  = opts("--reftobool")
    Mutable.refTypeof  = opts("--reftype")
    Mutable.refIsPrim  = opts("--refprim")
    Mutable.excRefine  = opts("--refexc")
    Mutable.refinement = opts("--refine")
    Mutable.dangle     = opts("--dangle")

    // make sure the options are all sanitized
    assert(opts.forall(o ⇒ {
      Set("--lightgc",
          "--fullgc",
          "--prune",
          "--testing",
    ...
    // we cannot do lightweight gc as well as full gc at the same time
  ...
  def optionToTrace(str: String): Trace = {
    ...
    str match {
      ...
      case _ ⇒ sys.error("Invalid trace option")
    }
  }
  ...
  def readAST( file:String ): Stmt = {
    val prog = RunTranslator.translateFileToProgram(
      new File(file), Mutable.testing || Mutable.print )
    Topo.order(prog) // topologically order the Merge nodes
    Topo.scratchOrder(prog) // give function IDs to scratch nodes
    prog
  }
...
case class State( t:Term, ρ:Env, σ:Store, ß:Scratchpad, κs:KStack, τ:Trace ) 
  ...
  def next: Set[State] = 
    try {
      t match {
        case StmtT( s:Stmt ) ⇒ /*StatsDeux.stmts(s);*/ s match { // !! STATS
          ...
          case If(e, s1, s2) ⇒ 
            // !! BRANCH
            if ( notJS.Mutable.dumb){StatsDeux.dumbError(e, ρ, σ, ß)}
            eval(e).b match {
              case Bool.True  ⇒ State(s1, ρ, σ, ß, κs, τ update s1)
              case Bool.False ⇒ State(s2, ρ, σ, ß, κs, τ update s2)
              case Bool.⊤     ⇒ {
                val (σT, ßT, σF, ßF) = branchRefine(e, σ, ρ, ß) 
                Set( State(s1, ρ, σT, ßT, κs, τ update s1), 
                                     State(s2, ρ, σF, ßF, κs, τ update s2) 
                                   )
              }
              case _          ⇒ Set()
            }
          ...
          case While(e, s) ⇒
            ...
            eval(e).b match {
              ...
              case Bool.⊤     ⇒ {
                val (σT, ßT, σF, ßF) = branchRefine(e, σ, ρ, ß) 
                advanceBV(Undef.BV, σF, ßF, κs) +
                                State(s, ρ, σT, ßT, κs push WhileK(e, s), τ update s)
              }
              case _          ⇒ Set()
            }
          ...
          case New(x, e1, e2) ⇒ 
            ...
            val exc = if (!bv1.defAddr) {
              ...
              Set(State(typeError, ρ, σ3, ß1, κs, τ))
            } else {
              Set[State]()
            }

            val (_σ3, _ß1) = excRefine(e1, σ3, ρ, ß1, FuncFilter)
          ...
          case ToObj(x, e) ⇒
            ...
            val s1 = noexc match {
              case Some((bv, σ1, ß1)) ⇒ {   
                val (_σ1, _ß1) = excRefine(e, σ1, ρ, ß1, UndefNullFilter)
            ...
            val s2 = exc match {
              case Some(ev) ⇒ {
                StatsDeux.except(s.id, ev)  // !! STATS
          ...
          case Update(e1, e2, e3) ⇒ 
            ...
            val s1 = noexc match {
              case Some((bv, σ1)) ⇒ {
                val (_σ1, _ß) = excRefine(e1, σ1, ρ, ß, UndefNullFilter)
          ...
          case Del(x, e1, e2) ⇒ 
            ...
            val s1 = noexc match {
              case Some((σ1, ß1)) ⇒ {
                val (_σ1, _ß1) = excRefine(e1, σ1, ρ, ß1, UndefNullFilter)
                advanceBV(Undef.BV, σ1, ß1, κs)
              }
              case None ⇒ Set()
            }
            val s2 = exc match {
              case Some(ev) ⇒ {
                /*if ( notJS.Mutable.stats ){Stats.raisedExc(e1);} */
                StatsDeux.except(s.id, ev) // !! STATS
                advanceEV(ev, ρ, σ, ß, κs, τ)
              }
              case None ⇒ Set()
          ...
          case Call(x, e1, e2, e3) ⇒ {
              val exc = if (!eval(e1).defAddr) {
                StatsDeux.except(x.id, typeError ) // !! STATS
                Set(State(typeError, ρ, σ, ß, κs, τ))
              } else {
                Set[State]()
              }
              val (_σ, _ß) = if (!notJS.Mutable.excRefine || exc.isEmpty){
                (σ, ß)
              } else {
                // do refinement
                excRefine(e1, σ, ρ, ß, FuncFilter)
              }

              exc ++ applyClo( eval(e1), eval(e2), eval(e3), x, ρ, _σ, _ß, κs, τ )
            }
    ...
    κs1.top match {
      ...
      case WhileK(e, s) ⇒
        ...
        Eval.eval(e, ρ, σ1, ß1).b match {
          ...
          case Bool.⊤     ⇒ {
            val (σT, ßT, σF, ßF) = branchRefine( e, σ1, ρ, ß1) 
            advanceBV(Undef.BV, σF, ßF, κs1.pop) + 
                            State(s, ρ, σT, ßT, κs1, τ update s)
          }
          case _          ⇒ Set()
        }
