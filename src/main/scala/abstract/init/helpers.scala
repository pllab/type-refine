...
object InitHelpers {
  ...
  def Convert
    ...
    /* convBValue: convert single base value by given hint, accounting
       ...
       contain. */
    def convBValue(hint: PrimHint, bv: BValue): BValue =
      bv.as.map(convObj(hint)).foldLeft(convPrim(hint)(bv))(_ ⊔ _)

    /* convObj: convert object at address by given hint */
    def convObj(hint: PrimHint)(a: Address): BValue = {
      ...
      lazy val valueOf = lookup(Set(a), SConstNotNum("valueOf"), σ)
      lazy val toString = lookup(Set(a), SConstNotNum("toString"), σ)
      val o = σ getObj a
      ...
      lazy val values = valueOf.as map { av ⇒ av match {
        ...
        case _ ⇒ sys.error("Your valueOf is not sane: " + av)
      } }
      ...
      lazy val strings = toString.as map { av ⇒ av match {
        ...
        case _ ⇒ sys.error("Your toString is not sane: " + av)
      } }
      ...
      // TODO: fix this back, make this a type error    
      convPrims(primaryConverted) ⊔
        (if (primaryConversion.defAddr) BValue.⊥ else
           convPrims(secondaryConverted))
             // (if (secondaryConversion.defAddr) BValue.⊥ else
             //   sys.error("TypeError in conversion to primitive: toString/valueOf not found")))
    }

    val prims = l map {
      case (bv, NoConversion) ⇒ bv
  ...
  def createFunctionObj(clo: Native, external: ExternMap = ExternMap(), internal: Map[Str, Any] = Map(), present: Set[Str]= Set(), cls: JSClass = CFunction): Object = {
    assert(external.exactnotnum contains SConstNotNum("length"), "functions must have length property")
    ...
    val present1 = (external.exactnotnum.keys ++ external.exactnum.keys).foldLeft(present)(_ + _)
    Object(external, internalFieldMap, present1)
  }
  ...
  def sigFunctionObj(sig: UsualSignature)(f: (List[BValue], Var, Env, Store, Scratchpad, KStack, Trace) ⇒ Set[State]): Object = {
    ...
    createFunctionObj(Native(
            ...
            val argsList: List[(BValue, ConversionHint)] = sig match {
              case Sig(selfHint, argHints, _) ⇒ {
                (selfAddr → selfHint) :: List.range(0, argHints.length).zip(argHints).map {
                  case (i, hint) ⇒ (argsArray(SConstNum(i.toString)) getOrElse Undef.BV, hint)
                }
              }
      ...
      ExternMap(exactnotnum = Map(
        SConstNotNum("length") → Num.inject(NConst(sig.lengthProperty))
      ))
    )
  }
  ...
  def genValueObjConstructor(cname: String)(bvtrans: BValue ⇒ BValue) =
    (bvs: List[BValue], x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ: Trace) ⇒ {
      ...
      val List(selfAddr_bv, arg_value) = bvs
        ...
        if (cname == "String") {
          ...
          val extern = exactStr match {
            case Some(s) ⇒ List.range(0, s.length).foldLeft(
              new_self.extern ++ (Fields.length → Num.inject(NConst(s.length))))(
                (acc, e) ⇒ acc ++ (Str.α(e.toString) → Str.inject(Str.α(s(e).toString))))
            case None ⇒ (new_self.extern + (Fields.length → Num.inject(NReal))) + (SNum → Str.inject(STop))
          }
  ...
  def dangleMap(m: Map[SConstNotNum, BValue]): Map[SConstNotNum, BValue] = {
