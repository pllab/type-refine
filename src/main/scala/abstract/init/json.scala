...
object InitJSON {
  ...
  val JSON_Obj = createObj(ExternMap(exactnotnum = Map(
    SConstNotNum("parse") → Address.inject(JSON_parse_Addr),
    SConstNotNum("stringify") → Address.inject(JSON_stringify_Addr)
  ))) // TODO: JSON class
  ...
  val JSON_parse_Obj = createFunctionObj(
    Native(
      (selfAddr: BValue, argArrayAddr: BValue, x: Var, ρ: Env, σ: Store, ß:Scratchpad, κ: KStack, τ:Trace) ⇒ {
        ...
        val text = argsArray(SConstNum("0")) getOrElse Undef.BV

        /* TODO: not currently handling second `reviver` argument
           as it can be a user function */
        if (argsArray(SConstNum("1")).nonEmpty)
