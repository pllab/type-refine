...
object PVarMapper {
  ...
  val argsVar = newMangledVar(ARGUMENTS_NAME)
  val argsName = getName(argsVar)
...
class Translator(val fromAst: JSAST) {
  ...
  val BREAK_LABEL = ":BREAK:" // TODO: Hoist these, also reorder everything. - LOW PRIORITY
  val CONTINUE_LABEL = ":CONTINUE:"					       
  val FUNCTION_TYPE = "function"

  val throwTypeError = Throw(StrAST(TYPE_ERROR))

  // TODO: the semantics state that "valueOf" should ignore passed parameters.  What - low priority
  ...
  def translate(j: JSAST): TranslationRetval = 
	...
	throw BadTransformation("Found compound assignment")
      ...
      case JSSimpleUpdate(JSAccess(e1, e2), e3) ⇒ {
        ...
        (asSeq(e1s, e2s, accessS, e3s, Update(obj, field, e3e)),
	...
	throw BadTransformation("Found compound update")
      ...
      case JSRegexp(JSStr(string)) ⇒ {
        val (sargs, eargs, eys) = makeArguments(Seq(StrAST(string)))
        val y = freshScratch
	...
	val pred = asSeq(sfp, asSeq(sip: _*))
	    ...
	    (asSeq(pred,
                   makeIf(Unop(IsPrim, efp),
	                  ...
	                  asSeq(argsStmt,
                                makeNew(y, efp, argsExp)))),
             y, e1ys ++ flattenVars(eiys) ++ argsys)
          }
        }
      }
	...
	val decl = Decl(declParams ++ declLocal ++ tempBindings,
			Lbl(RETURN_LABEL, asSeq(bodyS: _*))) // Lbl so we don't overwrite the return value
	...
	// TODO: the semantics document should say that these are, in fact, equal except
	// for the operators
	...
	def lessThanCore(stringCompare: Bop,
	  ...
	  val yRetval = freshScratch
	  // TODO: in the less than rule, the "string" string is missing the righthand '"' - low
	  // TODO: this is very different from the current translation document, and it
	  // follows ECMA closely
	...
	// TODO: formalism doesn't show short-circuiting behavior
	...
	op match {
	  ...
	  case JSGreaterThanOrEqual ⇒
	    lessThanCore(⌜≼⌝, ⌜≤⌝, true)
	  case JSIn ⇒ {
	    // TODO: more deviation from the spec; simplified but equivalent
	    // TODO: the semantics don't put `toString` in double quotes here
	...
	op match {
	  ...
	  case JSBitNot ⇒
	    negationHelper(⌞~⌟)
	  case JSLogNot ⇒
	    // TODO: is there a reason why the spec uses a scratch variable here?
	...
	val (fieldNames, fieldSs, fieldEs, fieldys) = 
          unzip4(
            fields.map(
	      { case (JSStr(name), jsE) ⇒ {
	        val (fe, fs, fys) = translate(jsE)
	        (StrAST(name), fe, fs, fys) } } ))
	...
	// TODO: what if e1 doesn't evaluate down to an object?
	val (e1S, e1E, e1ys) = translate(e1)
	val (e2S, e2E, e2ys) = translate(e2)
	val (fieldS, fieldE, fieldys) = toString(e2E)
        val deleteScratch = freshScratch
	val yRetval = freshScratch
        (asSeq(e1S, e2S, fieldS,
	       Del(deleteScratch, e1E, fieldE),
	       Assign(yRetval, deleteScratch)),
	 yRetval, e1ys ++ e2ys ++ fieldys)
      }

      case JSDelete(e) ⇒ {
        val (s, _, ys) = translate(e)
        (s, BoolAST(false), ys) // TODO: magic value
      }
	...
	val (spp, _, yspp) = translate(body)
	   ...
	   UndefAST(), ysp ++ iterableObjys ++ yspp)
        ...
        itervar match {
          case v: JSVar ⇒ 
	    // TODO: I'm adding the labels here, but the semantics don't show this - Ethan, formalism
	...
	val (catchVar, catchS, catchys) =
	  catchPart match {
	    case Some((JSPVar(name), e)) ⇒ {
              val (s, _, ys) = translate(e)
              (PVarMapper.getPVar(name), s, ys)
            }
	    case _ ⇒
	      // TODO: is using dummy going to mess anything up in the interpreter?
	      (PVarMapper.dummy, PVarMapper.noOpStatement, Set[PVar]())
	  }
	...
	val (spp, _, yspp) = default.map(translate).getOrElse(noOpRetval)
        // TODO: y0 is assigned but never used in the semantics -low
