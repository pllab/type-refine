...
import notjs.translator.PVarMapper.{selfName, windowName, argsName,
...
object HoistFunctions extends JSOnlyUpwardPass[List[JSFunctionDecl]] {
  ...
  def newBody(oldBody: JSStmt): JSStmt = {
    ...
    JSSeq(toHoist.reverse ++ List(ast))
  }
  ...
  override def atTransformEnd(j: JSAST, decls: List[JSFunctionDecl]): (JSAST, List[JSFunctionDecl]) =
    (JSSeq(decls.reverse ++ List(toStmt(j))), upwardBase)
...
object HoistGlobalVars extends JSOnlyDownwardPass[List[Set[JSPVar]]] {
  ...
  def ifIsArguments[X](name: String, scope: List[Set[JSPVar]], ifSo: JSPVar ⇒ X, ifNot: ⇒ X): X =
    if (name == ARGUMENTS_NAME && scope.size > 2)
      ifSo(JSPVar(argsName))
    else
      ifNot
  ...
  val simpleAssignTransform: Transformer =
    { case (JSSimpleAssign(x@JSPVar(name), rhs), scope) if !isInScope(x, scope) ⇒
        ...
        } yield ifIsArguments(name, scope,
                              (v: JSPVar) ⇒ JSSimpleAssign(v, rhsAst),
                              updateGlobally(name, rhsAst))
   }
  ...
  val tryTransform: Transformer =
    { case (JSTry(body, catchPart, finallyPart), scope) ⇒ {
        val (bodyAst, _) = transformStmt(body, scope)
        val catchPortion =
          catchPart.map(
            { case (x, s) ⇒ (x, transformStmt(s, Set(x) :: scope)._1) })
        val finallyPortion =
          finallyPart.map(s ⇒ transformStmt(s, scope))
        (JSTry(bodyAst,
               catchPart.map(
                 { case (x, s) ⇒ (x, transformStmt(s, Set(x) :: scope)._1) }),
  ...
  val varTransform: Transformer =
    { case (x@JSPVar(name), scope) if !isInScope(x, scope) ⇒
        ifIsArguments(name, scope,
                      (v: JSPVar) ⇒ (v, upwardBase),
