...
sealed abstract class Exp extends AST {
  // once the AST is created, we'll give each Exp identifiers
  // such that identifiers are equivalent within function boundaries.
  // in essence, canonicalize expressions by function boundaries
  var funcId = -1
}
...
// JS object classes
//
// !! TODO: need to add the special classes that are only in the init state
...
object Topo {
  ...
  // give all the scratch variables distinct function IDs
  def scratchOrder( s:Stmt ): Unit = {
    var ordnum = 0

    def number( node:AST ): Unit = {
      if (node.isInstanceOf[Exp]) {node.asInstanceOf[Exp].funcId = ordnum}

      node match {         
        case Newfun(x, Method(_, _, s), _) ⇒ number(x); ordnum += 1; number(s)

        case Decl(bs, s) ⇒ bs.map( pr ⇒ {number(pr._1); number(pr._2)}); number(s)
        case SDecl(_, s) ⇒ number(s)
        case Seq(ss) ⇒ ss.foreach( number(_) )
        case If(e, s1, s2) ⇒ number(e); number(s1); number(s2)
        case While(e, s) ⇒ number(e); number(s)
        case Try(s1, _, s2, s3) ⇒ number(s1); number(s2); number(s3)
        case Throw(e) ⇒ number(e)
        case Lbl(_, s) ⇒ number(s)
        case Jump(_, e) ⇒ number(e)
        case For(_, _, s) ⇒ number(s)
        case Assign(x, e) ⇒ number(x); number(e)
        case Call(x, e1, e2, e3) ⇒ number(x); number(e1); number(e2); number(e3)
        case New(x, e1, e2) ⇒ number(x); number(e1); number(e2)
        case ToObj(x, e) ⇒ number(x); number(e)
        case Del(x, e1, e2) ⇒ number(x); number(e1); number(e2)
        case Update(e1, e2, e3) ⇒ number(e1); number(e2); number(e3)

        case Binop(_, e1, e2) ⇒ number(e1); number(e2)
        case Unop(_, e) ⇒ number(e) 

        case Print(e) ⇒ number(e) 
